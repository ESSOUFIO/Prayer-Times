...
function TimeLeft(){
    ...
    if (HourNowSec > FajrSec && HourNowSec <= DhuhrSec){
        ...
        TimeDiff = TimeDifference(HourNowSec, MidnightSec) + TimeDifference(0, FajrSec)
    }
    ...
    if (LastPrayer === ""){ 
        ...
        document.getElementById(LastPrayer).style.backgroundColor = "rgba(1, 49, 53, 0.3)";
        LastPrayer = NextPrayer
    }
...
let getCountries = async () => {
    ...
    document.getElementById("countries").addEventListener("change", CountrySelected)
    CountrySelected() 
    document.body.style.cursor = "default"
}
...
let getCities = async (Country) => {
    let city = ""
    let PalestineCities = ["Jerusalem", "Bethlehem", "Beit Jala", "Beit Sahour", "Hebron",
        "Sabastia", "Jericho", "Ramallah", "Nablus", "Tulkarem", "Jenin",
...
function Calendar(){
    ...
    if (!TimesTable){ //Calendar displayed 
        ...
        document.getElementById("CalendarIcon").setAttribute("style", "color: greenyellow;")
